...
def contacts(): 
  ...
  nrOfTablesInInitialGroup = 4
  jinjaStringAndNrOfTablesAndNrOfImages = printHTMLtables(matchDict,nrOfTablesInInitialGroup)
  ...
  scrollingAreaWidth = 660*nrOfTablesInInitialGroup  + 40     # assumes 660 width for each table group
  print nrOfTables

  # serve up html page with matchDict
  return render_template('twinkedIn_demoBasic.html', 
                        matchDict = matchDict, 
                        jinjaString =Markup(jinjaString),
                        scrollingAreaWidth = scrollingAreaWidth,
                        jinjaTotalNrOfImages = nrOfImages,
                        totalNrOfTables = nrOfTables)
...
@app.route('/visualize/')
def visualize():
  return render_template('twinkedIn_demoAdvanced.html')
